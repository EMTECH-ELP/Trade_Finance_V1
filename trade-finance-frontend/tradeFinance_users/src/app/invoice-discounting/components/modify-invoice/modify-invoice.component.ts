...
export class ModifyInvoiceComponent implements OnInit {
 ...
 invoiceNumber: number;
  ...
  constructor(
    ...
    private authService: InvDiscountingService,
  ) { }

  ngOnInit(): void {
    // Initialize form fields with existing rows data
    this.invoiceNumber = this.rows.no;
    
    // this.lastName = this.rows.lastName;
    // this.email = this.rows.email;
    // this.phoneNo = this.rows.phoneNo;
    // this.rowsname = this.rows.rowsname;
    // this.role = this.rows.role[0].name;
  }
